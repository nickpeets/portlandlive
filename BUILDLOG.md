...
- **2026-09-23 — Name search covers all (d5386df0)** — filtered(): a search with words beyond a date skips the music-only gate (Mark Normand at Helium was unfindable); date-only searches stay music; Comedy section unchanged.
...
- **2026-09-25 — Clip upload failures shown (f56a78dc)** — the redraw after an upload erased the failure reason, so failed photos/clips looked like nothing happened (Charles Carroll, Parcels at Edgefield). Reason now persists; clip read times out at 15 s, frame grab at 6 s with a stand-in poster; clips capped at 50 MB (Supabase Free global limit); upload clock.

- **2026-09-25 — Feed health tiers + quiet venues (49df23ba)** — a zero venue's health line now shows what each fetch tier got (FETCH_NOTES in tls_rows: chrome/safari/browser). An all-past page (LaVerne's) reports "nothing upcoming posted yet (latest listing ...)" as kind quiet, no alert dot. Kelly's and Realm clear from the Codespace; the block is on GitHub's runner only.

- **2026-09-25 — Phone cards pass (38368f97)** — narrower poster (72px, 62px under 380px), date · time / venue / chips on separate lines via phone-only .s-brk spans, heart count on the heart button (.h-n) instead of the tally line, short "SAT OCT 31" dates on Heating Up cards, one-line banner and a "🔥 16" pill on the day header. Desktop unchanged except the banner wording ("in the past week").
