...
VENUE_INFO = {
    ...
    "Volcanic Theatre Pub": ("Bend", "70 SW Century Dr, Bend, OR 97702"),
    "Tower Theatre": ("Bend", "835 NW Wall St, Bend, OR 97703"),
    "Midtown Music Hall": ("Bend", "51 NW Greenwood Ave, Bend, OR 97701"),
    "Domino Room": ("Bend", "51 NW Greenwood Ave, Bend, OR 97701"),
    "45 East": ("Central Eastside", "315 SE 3rd Ave, Portland, OR 97214"),
...
# --- Feed health for the moderator menu (Sep 24 2026, Nick: yes to "tell me
# on day one"). Every SAFETY NET / EXPIRED / YIELD-ZERO line above only lands
# in the build log, which nobody reads; this is the same facts as data. It is
# written into manual_shows.json as "_health"; build_shows.py copies it into
# shows.json as "health", and the bookmark menu shows it to moderators.
...
def net_health(net, last_good, scraped, today, zero_reports=(), notes=None):
    ...
    from collections import Counter
    counts = Counter(s.get("venue", "") for s in scraped)
    drop = lambda since: (datetime.date.fromisoformat(since)
                          + datetime.timedelta(days=RETAIN_DAYS + 1)).isoformat() if since else ""
    items = []
    ...
    for v, (now, avg, extra, age) in sorted((net.get("partial") or {}).items()):
        items.append({"venue": v, "kind": "partial", "scraped": now, "usual": avg, "added": extra,
                      "since": last_good.get(v, ""), "drops": drop(last_good.get(v)), "days": age})
    reported = {i["venue"] for i in items}
    for v, since in sorted((last_good or {}).items()):
        if v in reported or counts.get(v) or v in SEASONAL_VENUES or not since:
            continue
        age = (today - datetime.date.fromisoformat(since)).days
        if RETAIN_DAYS < age <= DARK_REPORT_DAYS:
